test_that("both HLA-A alleles are extracted from beginning", {
  ...
  df_out <- tidyr::tibble(A_1 = "A1", A_2 = "A2", df_in)
  expect_equal(extract_alleles(df_in, typing), df_out)
})
...
test_that("second allele is NA when only 1 HLA-A allele is present", {
  ...
  df_out <- tidyr::tibble(A_1 = "A1", A_2 = NA_character_, df_in)
  expect_equal(extract_alleles(df_in, typing), df_out)
...
test_that("'A's in MACs are not extracted", {
  ...
  df_out <- tidyr::tibble(A_1 = "A*01:AABJE", A_2 = "A*02:HBMC", df_in)
  expect_equal(extract_alleles(df_in, typing), df_out)
})
